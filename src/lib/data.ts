export const navLinks = [
  { name: 'About', href: '#about' },
  { name: 'Projects', href: '#projects' },
  { name: 'Testimonials', href: '#testimonials' },
  { name: 'Contact', href: '#contact' },
];

export const projects = [
  {
    title: 'Project Alpha',
    description: 'A cutting-edge web application built with Next.js and TypeScript, featuring a highly interactive 3D interface using Spline. This project pushed the boundaries of web-based user experiences.',
    techStack: ['Next.js', 'TypeScript', 'Spline', 'Tailwind CSS'],
    imageUrl: 'https://placehold.co/800x600',
    aiHint: 'abstract technology',
    link: '#',
  },
  {
    title: 'Project Beta',
    description: 'A responsive e-commerce platform developed with React and Redux for state management. Integrated with Stripe for seamless payments and designed for a high conversion rate.',
    techStack: ['React', 'Redux', 'Stripe', 'Node.js'],
    imageUrl: 'https://placehold.co/800x600',
    aiHint: 'futuristic dashboard',
    link: '#',
  },
    {
    title: 'Project Gamma',
    description: 'A mobile-first social media application with real-time chat functionality powered by Firebase. The UI was crafted to be intuitive and engaging for a broad user base.',
    techStack: ['React Native', 'Firebase', 'GraphQL', 'Figma'],
    imageUrl: 'https://placehold.co/800x600',
    aiHint: 'modern application',
    link: '#',
  },
];

export const testimonials = [
  {
    name: 'Jane Doe',
    title: 'CEO, Innovate Inc.',
    review: "Sarthak's work is exceptional. He delivered a high-quality product on time and was a pleasure to work with. His technical skills and creativity are top-notch.",
    imageUrl: 'https://placehold.co/100x100',
    aiHint: 'professional portrait',
  },
  {
    name: 'John Smith',
    title: 'CTO, Tech Solutions',
    review: 'The 3D integration Sarthak implemented was a game-changer for our platform. His problem-solving abilities are remarkable. Highly recommended developer.',
    imageUrl: 'https://placehold.co/100x100',
    aiHint: 'corporate headshot',
  },
  {
    name: 'Emily White',
    title: 'Product Manager, Creative Co.',
    review: 'Working with Sarthak was a fantastic experience. He has a great eye for design and user experience, which brought our vision to life beautifully.',
    imageUrl: 'https://placehold.co/100x100',
    aiHint: 'creative professional',
  },
   {
    name: 'Michael Brown',
    title: 'Lead Engineer, Future Systems',
    review: 'Sarthak is a very skilled developer. He single-handedly improved our frontend performance by 40%. A true asset to any team.',
    imageUrl: 'https://placehold.co/100x100',
    aiHint: 'software engineer',
  },
];

export const galleryImages = [
  { src: 'https://placehold.co/600x400', alt: 'Design Tile 1', hint: 'abstract art' },
  { src: 'https://placehold.co/600x400', alt: 'Design Tile 2', hint: 'futuristic city' },
  { src: 'https://placehold.co/600x400', alt: 'Design Tile 3', hint: 'digital render' },
  { src: 'https://placehold.co/600x400', alt: 'Design Tile 4', hint: 'glowing shapes' },
  { src: 'https://placehold.co/600x400', alt: 'Design Tile 5', hint: 'neon lines' },
  { src: 'https://placehold.co/600x400', alt: 'Design Tile 6', hint: 'cybernetic patterns' },
];
